...
class MetricComputer(object):
    ...
    def __call__(self, src_str, other, tgt_str):
        ...
        for metric in self.metrics:
            if metric.name != "Attention MSE metric":
                metric.eval_batch(other, targets, None)
    ...
    def __call__(self, src_str, tgt_str=None, attention_target=None):
        ...
        if tgt_str is not None:
            ...
            if self.is_symbol_rewriting and is_output_good_length:
                ...
                try:
                    metrics = self.metric_computer(full_src_str, other, full_tgt_str)
